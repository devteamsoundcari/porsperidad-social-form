...
class MultiSelect {
  ...
  /**
   * Establece los valores seleccionados en el multi-select.
   * @param {Array} values - Array de valores a seleccionar.
   */
  setValue(values) {
    // Asegúrate de que 'values' es un array
    if (!Array.isArray(values)) {
      throw new Error("El argumento debe ser un array.");
    }

    // Primero, deselecciona todas las opciones
    this.element.querySelectorAll(".multi-select-option").forEach((option) => {
      option.classList.remove("multi-select-selected");
      this.element
        .querySelector(`input[value="${option.dataset.value}"]`)
    ...
    // Luego, actualiza el estado de selección de las opciones
    values.forEach((value) => {
      let option = this.element.querySelector(
      ...
      if (option) {
        ...
        this.element
          ...
          .insertAdjacentHTML(
            "afterbegin",
            `<span class="multi-select-header-option" data-value="${value}">${
              option.querySelector(".multi-select-option-text").innerHTML
            }</span>`
          );
        ...
        this.data.filter((data) => data.value == value)[0].selected = true;
      }
    });

    // Actualiza el estado del encabezado
    this._updateHeader();

    // Ejecuta el evento de cambio
    this.options.onChange(null, null, null);
  }

  /**
   * Actualiza el estado del encabezado del multi-select.
   * Este método puede ser llamado después de que los valores sean establecidos para actualizar el encabezado.
   */
  _updateHeader() {
    let headerElement = this.element.querySelector(".multi-select-header");
    headerElement
      .querySelectorAll(".multi-select-header-option")
      .forEach((headerOption) => headerOption.remove());
    ...
    if (headerElement.querySelector(".multi-select-header-option")) {
      headerElement.querySelector(".multi-select-header-placeholder")?.remove();
    } else if (
      !headerElement.querySelector(".multi-select-header-placeholder")
    ) {
      headerElement.insertAdjacentHTML(
        "afterbegin",
        `<span class="multi-select-header-placeholder">${this.placeholder}</span>`
    ...
    if (this.options.max) {
      headerElement.querySelector(".multi-select-header-max").innerHTML =
        this.selectedValues.length + "/" + this.options.max;
    }
  }
}
